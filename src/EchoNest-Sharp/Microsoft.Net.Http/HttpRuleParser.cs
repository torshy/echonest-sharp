...
namespace System.Net.Http
{
    internal static class HttpRuleParser
    {
        ...
        private const string rfc850Format = "dddd, dd'-'MMM'-'yy HH:mm:ss 'GMT'";
        ...
        {
            // First try RFC1123 format (by far the most common and preferred format)
            if (DateTimeOffset.TryParseExact(input, "r", DateTimeFormatInfo.InvariantInfo,
                DateTimeStyles.AllowWhiteSpaces | DateTimeStyles.AssumeUniversal, out result))
            {
                return true;
            }

            // Try the next common format: RFC850
            if (DateTimeOffset.TryParseExact(input, rfc850Format, DateTimeFormatInfo.InvariantInfo,
                DateTimeStyles.AllowWhiteSpaces | DateTimeStyles.AssumeUniversal, out result))
            {
                return true;
            }

            // HTTP also supports ANSI C's asctime() format
            if (DateTimeOffset.TryParseExact(input, ascTimeFormat, DateTimeFormatInfo.InvariantInfo,
                DateTimeStyles.AllowWhiteSpaces | DateTimeStyles.AssumeUniversal, out result))
            {
                return true;
            }
